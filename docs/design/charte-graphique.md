...
- **`ActionsMenu`** (`src/app/comptes/[id]/edit.tsx`, ticket #45) : liste verticale des actions disponibles (Modifier, Supprimer, actions spécifiques comme « Marquer absente »), chaque action une ligne pleine largeur sur fond `backgroundElement` (`minHeight: 44`, radius `Spacing.two`) — même traitement que les autres lignes/cards de l'écran, jamais de texte nu sans support visuel —, pied Annuler seul. L'action destructive reste en texte `danger` (pas de fond dédié : un fond `backgroundSelected`, identique aux actions positives, brouillerait le signal destructif). Remplace l'`Alert.alert` natif ouvert par `ActionsMenuButton` (bouton « ⋮ »).
