...
- **`ActionsMenu`** (`src/app/comptes/[id]/edit.tsx`, ticket #45) : toutes les actions disponibles (Annuler en tête, puis Modifier, Marquer absente, Supprimer selon la ligne) sur une seule ligne. Annuler reste un lien texte neutre (comme dans AjoutPopup) ; chaque autre action est un **vrai bouton** identifiable visuellement — fond `backgroundElement`, radius `Spacing.two` — pas un simple lien texte flottant. L'action principale (`primaire: true` sur l'entrée `ActionMenuItem`, ex. Modifier) reprend la recette du bouton de validation d'AjoutPopup : fond `backgroundSelected`, texte `smallBold`. L'action destructive (Supprimer) reste en texte `danger` sur fond `backgroundElement` (jamais `backgroundSelected`, qui brouillerait le signal destructif sous un libellé rouge). `flexWrap` en filet de sécurité si les 4 boutons (cas niveau 3, avec « Marquer absente ») ne tiennent pas sur un très petit écran. Remplace l'`Alert.alert` natif ouvert par `ActionsMenuButton` (bouton « ⋮ »).
- **`ConfirmationSuppression`** (`src/app/comptes/[id]/edit.tsx`, ticket #45) : titre + message, pied Annuler (lien texte) / Supprimer (vrai bouton, fond `backgroundElement`, texte `smallBold` `danger` — même recette que l'action destructive d'`ActionsMenu`). Remplace l'`Alert.alert` natif déclenché depuis l'entrée « Supprimer » d'`ActionsMenu`.
