...
- **`ActionsMenu`** (`src/app/comptes/[id]/edit.tsx`, ticket #45) : toutes les actions disponibles (Annuler en tête, puis Modifier, Marquer absente, Supprimer selon la ligne) sur une seule ligne de liens texte alignée à droite — même traitement qu'Annuler/Supprimer dans `ConfirmationSuppression` — plutôt qu'empilées verticalement. L'action destructive (Supprimer) reste en `danger`, les autres en couleur par défaut ; `flexWrap` en filet de sécurité si les 4 libellés (cas niveau 3, avec « Marquer absente ») ne tiennent pas sur un très petit écran. Remplace l'`Alert.alert` natif ouvert par `ActionsMenuButton` (bouton « ⋮ »).
