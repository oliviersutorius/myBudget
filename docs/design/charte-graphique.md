...
- **Carte niveau 1** (Fixe/Variable) : même style que le pavé niveau 1 de l'onglet Dépenses (fond `backgroundElement`, radius `Spacing.three`, en-tête `paveHeader`), mais sans chevron ni bouton « + ». Sous l'en-tête, un groupe par type niveau 2 (en-tête `niveau2Header`, sans chevron/bouton non plus) puis ses lignes niveau 3 (`niveau3Row`/`niveau3RowMain`, indentées, séparateur fin entre lignes).
- **Carte Revenus** : même traitement que la carte niveau 1 (en-tête + lignes), sans indentation (pas de sous-hiérarchie).
- **Pas de carte/ligne vide** : une carte niveau 1 sans aucun type de dépense défini n'est pas rendue ; un groupe niveau 2 sans montant enregistré ce mois-ci n'est pas rendu ; la carte Revenus n'est rendue que s'il existe au moins un revenu ce mois. Contrairement aux écrans d'édition (Dépenses), les lignes niveau 3 sans montant ce mois (absentes/non saisies) sont omises plutôt qu'affichées avec un texte de substitution — un récapitulatif liste ce qui a été dépensé, pas ce qui reste à saisir.
