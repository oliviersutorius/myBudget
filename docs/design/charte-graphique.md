...
Introduite par le ticket #63 pour le détail d'un mois de l'onglet Budget (`BudgetTab`, `src/app/comptes/[id]/edit.tsx`) : un écran de consultation pure (aucune action ⋮/+, rien n'est collapsable) réutilise le vocabulaire visuel des pavés éditables de l'onglet Dépenses (#41) plutôt que d'introduire un nouveau style de carte — 3 maquettes proposées (canvas Claude Design, référencé par le ticket), maquette « B — Cartes » retenue puis réimplémentée avec l'échelle `ThemedText`/`Spacing` existante plutôt que ses valeurs pixel (même choix que #41, voir « Espacements » ci-dessous).
...
- **Carte niveau 1** (Fixe/Variable) : même style que le pavé niveau 1 de l'onglet Dépenses (fond `backgroundElement`, radius `Spacing.three`, en-tête `paveHeader`), mais sans chevron ni bouton « + ». Sous l'en-tête, un groupe par type niveau 2 (en-tête `niveau2Header`, sans chevron/bouton non plus, libellé et sous-total tous deux en `smallBold`) puis ses lignes niveau 3 (`niveau3Row`/`niveau3RowMain`, indentées, en `textSecondary` — seul endroit de cette carte à sortir du fond `backgroundElement`/texte `text` par défaut, pour distinguer visuellement le détail du sous-total niveau 2 juste au-dessus), séparées par un filet `backgroundSelected` (un ton au-dessus du fond de carte, pour rester visible dessus — `backgroundElement` serait invisible sur son propre fond).
- **Carte Revenus** : même traitement que la carte niveau 1 (en-tête + lignes), sans indentation ni `textSecondary` (pas de sous-hiérarchie à distinguer, contrairement aux lignes niveau 3).
