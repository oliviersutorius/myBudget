...
- **Une seule carte, un seul fond, du haut en bas** : contrairement à `Niveau2Ligne` (onglet Dépenses, #41), qui affiche _volontairement_ une carte imbriquée fond `background` sur fond `backgroundElement` (effet « encastré »), ce récapitulatif vise une carte plate d'un seul tenant — même fond `backgroundElement` sur l'en-tête, les groupes niveau 2 et les lignes niveau 3, sans rupture de fond. **Piège** : `ThemedView` retombe sur `background` (pas transparent) dès que `type` n'est pas fourni (`src/components/themed-view.tsx`) — un oubli de `type="backgroundElement"` sur un conteneur imbriqué de cette carte redessine silencieusement l'effet « encastré » de Dépenses au lieu du fond plat voulu ici (régression repérée sur device après le premier merge de #63, corrigée en passant `type="backgroundElement"` explicitement sur chaque `ThemedView` de l'arbre). Vérifier ce point sur device (pas seulement en relisant le code) à chaque nouvel écran de consultation en carte plate.
